    ...
    <div className="border border-border rounded-lg overflow-hidden bg-card text-card-foreground mb-6">
      ...
      {(!contacts || contacts.length === 0) ? (
        ...
        {contacts.map(contact => {
          ...
          return (
            <div key={contact.id} onClick={() => navigate(`/contacts/${contact.id}`)} className="flex flex-col p-3 bg-transparent border-r border-b border-border transition-colors hover:bg-muted/30 group h-[148px] justify-between cursor-pointer">
              ...
              <div className="flex flex-col items-center gap-2.5 text-center flex-1 justify-center mt-2">
              ...
              <div className="w-full text-right mt-auto">
                <span className={cn("text-xl font-mono tracking-tight font-normal truncate block", balance === 0 ? "text-muted-foreground" : balance > 0 ? "text-emerald-500" : "text-destructive")}>
